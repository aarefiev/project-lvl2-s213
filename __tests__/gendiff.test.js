...
fileContentsTypesList.forEach((type) => {
  fileExtensionsList.forEach((ext) => {
    describe(`diff for ${ext} files with ${type} content`, () => {
      ...
      it(`${file1} with ${empty}`, () => {
        const expected = fs
          .readFileSync(preparePath(`answer-before-with-empty-${type}`), encoding);
      ...
      it(`${empty} with ${file1}`, () => {
        const expected = fs
          .readFileSync(preparePath(`answer-empty-with-before-${type}`), encoding);
      ...
      it(`${file1} with ${file2}`, () => {
        const expected = fs
          .readFileSync(preparePath(`answer-before-with-after-${type}`), encoding);
      ...
      it(`${file2} with ${file1}`, () => {
        const expected = fs
          .readFileSync(preparePath(`answer-after-with-before-${type}`), encoding);
